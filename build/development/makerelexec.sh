...
# - RELEXEC_NODE_PATH - [optional] path to the node binary, if required to ensure ldd can resolve all library paths in .node files
# - RELEXEC_EXTRA_SYSTEM_LIB_PATHS - [optional] list of extra system library paths to be added to the RPATH
...
LD_PATH=$(ls -1 /lib/ld-musl-* /lib/*-linux-gnu/ld-linux* 2>/dev/null | head -n 1)
...
_checkelfs() {
  ...
  do
    ...
    $RELEXEC_CODE_PATH$LD_PATH --list $lib 2>/dev/null | sed -nr '/=>/!d; s/^\s*(\S+)\s*=>\s*(.*?)(\s*\(0x[0-9a-f]+\))?$/- \2 \1/;/^.+$/p;' | egrep -v "^- ($RELEXEC_CODE_PATH/|$RELEXEC_EXEC_PATH/.*/$LD_BIN)"
...
patch_binary() {
  ...
  if patchelf --set-interpreter $RELEXEC_EXEC_PATH$LD_PATH $bin 2>/dev/null; then
    echo patchelf --set-interpreter $RELEXEC_EXEC_PATH$LD_PATH $bin >>$TMP/patchelf.log
    return 0
  fi
...
generate_system_lib_paths() {
  ...
  # $RELEXEC_CODE_PATH$LD_PATH --list-diagnostics | grep ^path.system_dirs | sed -r 's|^.*="([^"]+)/?"$|\1|; s|/$||' | sort -u

  # This list is generated by:
  # - Extracting the path to each library, relative to $RELEXEC_CODE_PATH; add leading '/' if missing.
  cat "$@" | \
    grep -E '\.so(\.[0-9]+)*$' | \
    sed -r "s|^$RELEXEC_CODE_PATH||; s|/[^/]+$||; s|^[^/]|/|;" | \
    grep -E '^(/usr|/lib)(/|$)' | \
    sort -u
}
...
all() {
  ...
  cp --parents $LD_PATH $RELEXEC_CODE_PATH
  ln -s $(echo $LD_PATH | sed -r 's|^/lib/|./|') $RELEXEC_CODE_PATH/lib/ld
}
