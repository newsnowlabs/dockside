...
usage() {
  echo "$0: [[--stage <stage>] [--tag <tag>]] [--push|--load] [--no-cache] [--force-rm] [--progress-plain] [--repo <repo>] [--builder [depot|buildx|buildkit]] [--platform=<platforms>] | [--clean] | [--list]" >&2
  exit
}
...
build_env() {
  ...
  DOCKER_OPTS+=(--add-host=rubygems.org:151.101.193.227)
  DOCKER_OPTS+=("--label=com.newsnow.docksideio.build.date=$TAG_DATE")
  ...
  [ -n "$TAG" ] && DOCKER_OPTS+=("--label" "com.newsnow.docksideio.build.tag=$TAG")
