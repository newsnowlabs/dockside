...
> **Tip:** Profiles may specify `"networks": ["*"]` to automatically present the user with all Docker networks currently connected to the Dockside container. This means new networks become available to devtainers as soon as Dockside is connected to them, with no profile update required.
