...
- Anything else: after stripping any leading `#` (so `42` and `#42` are equivalent), purely numeric → a PR number, running `gh pr checkout <ref>` (falling back to `git fetch origin refs/pull/<ref>/head && git checkout FETCH_HEAD` if `gh` is unavailable or that fails); otherwise a branch name, fetched and switched to from `origin`, failing loudly rather than silently creating an empty local branch if it doesn't exist there.
