...
This checkout only happens when a repository was actually cloned, i.e. when the profile's `gitURLs` is non-empty *and* the user supplies a `gitURL` at launch. For profiles with no `gitURLs`, or when the `gitURL` field is left blank at launch, no cloning or branch/PR switching occurs — the `branch`/`pr` fields still populate `DOCKSIDE_OPTION_BRANCH`/`DOCKSIDE_OPTION_PR` like any other `options` entry (see the `options` row above), but those values have no special meaning unless the profile's own `command`/`entrypoint`/[hook](extensions/lifecycle-hooks.md) chooses to consume them. See [Lifecycle hooks](extensions/lifecycle-hooks.md) for the full set of patterns — including this one — for making a launched devtainer switch to (and actually run) a requested branch or PR, and how to pick between them.
