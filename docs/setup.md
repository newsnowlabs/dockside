...
The example [`03-git-repo.json`](https://github.com/newsnowlabs/dockside/blob/main/app/server/example/config/profiles/03-git-repo.json) profile builds on `gitURLs` clone-on-launch by adding a single `options` entry with the reserved name `ref`, which may hold either a branch name or a PR number. This name is special-cased by `launch.sh`: once the launch-time `gitURL` has been cloned, it inspects the `ref` value — after stripping any leading `#` (so `42` and `#42` are equivalent) — and if what's left is purely numeric, treats it as a PR number, running `gh pr checkout <ref>` (falling back to `git fetch origin refs/pull/<ref>/head && git checkout FETCH_HEAD` if `gh` is unavailable or that fails); otherwise it treats `ref` as a branch name, fetching and switching to it from `origin`, failing loudly rather than silently creating an empty local branch if the branch doesn't exist there.
