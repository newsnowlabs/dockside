...
The example [`03-git-repo.json`](https://github.com/newsnowlabs/dockside/blob/main/app/server/example/config/profiles/03-git-repo.json) profile builds on `gitURLs` clone-on-launch by adding a single `options` entry with the reserved name `ref`, which may hold a branch name, a PR number, or a full GitHub URL copied straight from the browser. This name is special-cased by `launch.sh`: once the launch-time `gitURL` has been cloned, it interprets `ref` as follows, in order:

- `https://github.com/<org>/<repo>/pull/<n>` (any trailing path/query/fragment, e.g. `/files`, `?diff=split`, ignored) — treated as PR `<n>`.
- `https://github.com/<org>/<repo>/tree/<ref>` or `.../commits/<ref>` — treated as a branch. Since a GitHub branch name may itself contain `/`, the URL is inherently ambiguous between "the whole remainder is the branch name" and "part of it is a subdirectory the URL is deep-linking into" — `launch.sh` resolves this by trying progressively shorter prefixes of the remainder against `origin`'s actual branches (via `git ls-remote`) until one matches, starting with the full remainder (which is what matches immediately for the common case: a URL with no subpath). This isn't a guess: git's ref namespace is hierarchical, so `feature` and `feature/foo` can never both exist as branches at the same time, meaning there is exactly one real branch any given remainder could resolve to, and the search finds it.
- Anything else: after stripping any leading `#` (so `42` and `#42` are equivalent), purely numeric → a PR number, running `gh pr checkout --force <ref>` (falling back to `git fetch origin refs/pull/<ref>/head && git checkout FETCH_HEAD` if `gh` is unavailable or that fails); otherwise a branch name, fetched and reset (`git reset --hard`) onto `origin`'s tip, failing loudly rather than silently creating an empty local branch if it doesn't exist there.

This checkout only happens when a repository was actually cloned, i.e. when the profile's `gitURLs` is non-empty *and* the user supplies a `gitURL` at launch — and, since `ref` is fixed for the reservation's whole lifetime and can never change after creation, only on the devtainer's genuine first start; a later restart leaves an already-checked-out ref alone. `--force`/`reset --hard` rather than a fast-forward-only merge is safe specifically because of that one-shot guarantee: a fresh devtainer has no prior local work of the user's own on the requested ref to lose, so a pre-existing local branch of that name (e.g. one baked into the image) converges unconditionally onto the just-fetched remote tip even if it had diverged, rather than failing loudly. For profiles with no `gitURLs`, or when the `gitURL` field is left blank at launch, no cloning or ref switching occurs — the `ref` field still populates `DOCKSIDE_OPTION_REF` like any other `options` entry (see the `options` row above), but that value has no special meaning unless the profile's own `command`/`entrypoint`/[hook](extensions/lifecycle-hooks.md) chooses to consume it. See [Lifecycle hooks](extensions/lifecycle-hooks.md) for the full set of patterns — including this one — for making a launched devtainer switch to (and actually run) a requested branch or PR, and how to pick between them.

> **Why doesn't `launch.sh` also switch a pre-existing repo when `gitURLs` isn't used?** It deliberately doesn't fall back to acting on a repo already present in the image (or left over from a previous launch) when there's no fresh `GIT_URL` clone to anchor it to. An application's own entrypoint could already be using that repo — mid-build, or about to start a service from it — and a checkout landing underneath it from a separate process would risk leaving the working tree in an indeterminate state. The [entrypoint and hook patterns](extensions/lifecycle-hooks.md) exist precisely so the *application* stays in control of when a switch happens, rather than something external doing it to a repo the app doesn't know is changing.
