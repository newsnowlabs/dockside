...
def build_proxy_command(cookie_header, wss_url):
    """
    Build a wstunnel ProxyCommand string matching the format generated by SSHInfo.vue.
    """
    # OpenSSH applies percent-expansion to the entire ProxyCommand line, so any
    # literal '%' present in the cookie header must be doubled first.
    cookie_header = f'Cookie: {cookie_header}'.replace('%', '%%')
    return (
...
class SshTestMixin:
    ...
    def _get_ssh_host_pattern(self):
        """
        Return (ssh_alias, ssh_hostname, wss_url) for the SSH config.

        local/harness: ssh_alias    = ssh-<name>.<domain-suffix>
                       ssh_hostname = <server-hostname>
                       wss_url      = wss://localhost:<port>
        remote:        ssh_alias    = ssh-<name><parentFQDN>
                       ssh_hostname = <server-hostname>
        ...
        if self.dev1._connect_to:
            ...
            ssh_alias = f'ssh-{self.SSH_CONTAINER}.{domain_sfx}'
            ...
            ssh_alias = f'ssh-{self.SSH_CONTAINER}{parent_fqdn}'
            wss_url = f'wss://{server_hostname}'

        return ssh_alias, server_hostname, wss_url
