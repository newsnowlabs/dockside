...
| `14_hooks.py` | Profile-declared `hooks.launch` lifecycle hook: auto-invoked once at launch, re-invoked on demand via `dockside hook run`, failure surfaces via both the in-container `.hook-failed` sentinel and a CLI-level error, and a profile with no hook configured is rejected cleanly |
