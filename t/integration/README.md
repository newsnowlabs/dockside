...
| `14_hooks.py` | Profile-declared hooks: `hooks."lifecycle:launch"` auto-invoked once at launch, re-invoked on demand via `dockside hook run <devtainer> <hook-name>` (required, no default) only when that hook's own entry sets `"manual": true`; a custom (non-lifecycle) hook needs no `"manual": true` entry and dispatches independently (separate sentinel/lock); failure surfaces via both the in-container `.hook-failed.<name>` sentinel and a CLI-level error; hook-naming edge cases (bare `"launch"` is inert, reserved-but-unimplemented names rejected regardless of `manual`) and schema validation (`HookNamingValidationTests`); and (`HookWithGitUrlTests`) a `gitURLs` profile with a non-`ref`-named option correctly falls through to a `lifecycle:launch` hook that switches the already-cloned repo to a branch or PR |
| `15_entrypoint_signaling.py` | Dockside's own signaling for the entrypoint-based branch/PR-switching patterns (not the application-level ref-parsing logic, already covered via pattern A in `06_git_profile.py`): `{option.<name>}` resolves verbatim into the container's own `command` argv at create time with no shell reinterpretation; once `/tmp/dockside/.credentials-ready` appears, a live (discovered, not pinned) ssh-agent socket genuinely has the launching user's key loaded, and (PR test requires `DOCKSIDE_TEST_GITHUB_TOKEN`) `gh` is genuinely authenticated on disk |
...
the CLI routes to `localhost` rather than via the public hostname. Authenticate
first so the stored server entry supplies the `--connect-to` transport:
```bash
dockside login \
...
The harness creates an isolated temporary CLI config directory, logs in there,
and drives the harness target through the CLI's stored server entry:
```bash
...
`DOCKSIDE_TEST_MODE=local` explicitly and ensure the stored CLI server entry
(set via `dockside login --connect-to 127.0.0.1`) supplies the TCP override.
In harness mode, `harness.sh` creates an isolated CLI config and runs
`dockside login --connect-to localhost:<port>` automatically.
...
| `DOCKSIDE_TEST_CA_BUNDLE` | — | Host path to a CA bundle (e.g. `/etc/ssl/certs/ca-certificates.crt`) to bind-mount read-only into every test profile at the same path. Devtainers launched by Dockside do not inherit the host's trust store, so this is needed behind a TLS-inspecting proxy whose CA is trusted on the host but not baked into devtainer base images — otherwise `apk`/`apt` package installs inside the devtainer fail with untrusted-certificate errors. Note this does **not** cover `06_git_profile.py`'s HTTPS clones: those run through Dockside's bundled `git` wrapper, which hardcodes `-c http.sslcainfo=<IDE_PATH>/certs/ca-certificates.crt` (a separate, image-baked file in the shared IDE volume, unrelated to the system trust store) — fixing that requires replacing that file's contents instead. |
...
- **`use_cli_admin_creds=True`** (used for the admin client in all modes): the
  CLI's pre-existing stored session is used — no credentials are passed. Requires
  a prior `dockside login` (done automatically by `harness.sh` in harness mode,
  and manually in local/remote modes).

- **`use_cli_admin_creds=False`** (default): explicit `--username`/`--password`
  flags are passed to the CLI on every call; a per-client temporary cookie file
  isolates the session. Used for all test-user clients (dev1/dev2/viewer).
...
which encapsulates non-trivial logic: TCP-override routing (via the stored
server entry's `connect_to`), cross-domain cookie injection, and JSON output.
This logic cannot be exercised by direct urllib calls. Using `check-url` in
the authenticated tests is the only way to get integration coverage of it.
...
`parent` chain, `use_cli_admin_creds`) and the requirement for a pre-configured
`dockside login` exist because the harness delegates session and transport
management to the CLI. A direct HTTP harness would manage cookies entirely
in-process with no such complexity.
