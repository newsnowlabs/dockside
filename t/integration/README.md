...
| `14_hooks.py` | Profile-declared hooks: `hooks."lifecycle:launch"` auto-invoked once at launch, re-invoked on demand via `dockside hook run <devtainer> <hook-name>` (required, no default) only when listed in `manualHooks`; a custom (non-lifecycle) hook needs no `manualHooks` entry and dispatches independently (separate sentinel/lock); failure surfaces via both the in-container `.hook-failed.<name>` sentinel and a CLI-level error; hook-naming edge cases (bare `"launch"` is inert, reserved-but-unimplemented names rejected regardless of `manualHooks`) and schema validation (`HookNamingValidationTests`); and (`HookWithGitUrlTests`) a `gitURLs` profile with a non-`ref`-named option correctly falls through to a `lifecycle:launch` hook that switches the already-cloned repo to a branch or PR |
