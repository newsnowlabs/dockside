...
| `14_hooks.py` | Profile-declared `hooks.launch` lifecycle hook: auto-invoked once at launch, re-invoked on demand via `dockside hook run`, failure surfaces via both the in-container `.hook-failed` sentinel and a CLI-level error, a profile with no hook configured is rejected cleanly, and (`HookWithGitUrlTests`) a `gitURLs` profile with a non-`ref`-named option correctly falls through to a hook that switches the already-cloned repo to a branch or PR |
